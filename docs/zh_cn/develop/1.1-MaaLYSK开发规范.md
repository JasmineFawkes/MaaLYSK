...
| **./minigame/** | 快乐出游小游戏，使用中文拼音或英文翻译 | `catcard_collect`、`weekly_game_catchtoy.json` |
...
> - daily、weekly里放的是**仅为完成日常/周常而设定**的任务，像快乐出游、战斗、限时活动这种专门的板块，为了便于统一查找，一般在**独立文件夹**存放，但可以利用**对单个json加前缀**的方式区别其属于日常/周常任务，如`weekly_game_catchtoy.json`、`event_daily_get.json`等。
>
...
目前多复用节点大致分为三类：菜单点击节点、右上角返回节点、弹窗节点。
...
| **popop.json**   | 各类弹窗节点<br>（牵绊度提升、获取奖励）                     | `退出xxpopop`            | `退出牵绊度popop`<br>`退出获取奖励popop`<br>`退出体力不足popop` |
